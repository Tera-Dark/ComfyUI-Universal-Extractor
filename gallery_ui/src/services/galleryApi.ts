...
export const galleryApi = {
  ...
  async getUpdateStatus(force = false) {
    ...
    return requestJson<UpdateStatus>(`/universal_gallery/api/update-status${query}`);
  },
  ...
  async listVariantGroups(options: {
    ...
    if (options.subfolder?.trim()) params.set("subfolder", options.subfolder.trim());
    if (options.boardId?.trim()) params.set("board_id", options.boardId.trim());
    if (options.dateFrom?.trim()) params.set("date_from", options.dateFrom.trim());
    if (options.dateTo?.trim()) params.set("date_to", options.dateTo.trim());
    if (options.favoritesOnly) params.set("pinned", "true");
    if (options.colorFamily?.trim()) params.set("color_family", options.colorFamily.trim());
    if (options.sortBy) params.set("sort_by", options.sortBy);
    if (options.sortOrder) params.set("sort_order", options.sortOrder);
    if (options.type) params.set("type", options.type);
    if (options.limit) params.set("limit", String(options.limit));
    if (options.forceRefresh) params.set("force_refresh", "true");
    return requestJson<VariantGroupResponse>(`/universal_gallery/api/images/variant-groups?${params}`);
  },

  async getVariantGroupImages(options: {
    groupId: string;
    type?: VariantGroupType | "";
    page?: number;
    limit?: number;
    ...
    if (options.sortBy) params.set("sort_by", options.sortBy);
    if (options.sortOrder) params.set("sort_order", options.sortOrder);
    return requestJson<VariantGroupImagesResponse>(`/universal_gallery/api/images/variant-group?${params}`);
  },

  async prewarmFingerprints(options: {
    ...
    return requestJson<{
      ...
      failed: number;
      last_error: string;
      status: VariantGroupResponse["fingerprint_status"];
    }>(
      "/universal_gallery/api/images/fingerprints/prewarm",
      {
        ...
        body: JSON.stringify({
          relative_paths: options.relativePaths,
          ...
          pinned: options.favoritesOnly,
          color_family: options.colorFamily,
          limit: options.limit,
        }),
      },
    );
  ...
  async getFingerprintStatus() {
    return requestJson<VariantGroupResponse["fingerprint_status"]>(
      "/universal_gallery/api/images/fingerprints/status",
    );
  },

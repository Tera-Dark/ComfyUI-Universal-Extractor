...
const renderWorkspace = (overrides: Partial<Parameters<typeof GalleryWorkspace>[0]> = {}) => {
  ...
  const props: Parameters<typeof GalleryWorkspace>[0] = {
    ...
    onApplyLoraStack: vi.fn().mockResolvedValue(undefined),
...
describe("GalleryWorkspace smoke", () => {
  ...
  it("offers the LoRA stack apply action from the image context menu", () => {
    const onApplyLoraStack = vi.fn().mockResolvedValue(undefined);
    window.localStorage.setItem("universal-extractor:gallery-view-mode", "list");
    const { container } = renderWorkspace({
      isTrashView: false,
      selectedSubfolder: "default_output::",
    ...
    expect(onApplyLoraStack).toHaveBeenCalledWith(expect.objectContaining({ filename: "sample.png" }));
  });

...
export const ImageDetailModal = ({
  ...
  useEffect(() => {
    let cancelled = false;
    galleryApi
      .listVariantGroups({ subfolder: image.subfolder, limit: 80 })
      .then((response) => {
        if (cancelled) {
          return;
        }
        const related = response.groups.filter((group) =>
          group.images_preview.some((preview) => preview.relative_path === image.relative_path),
        );
        setRelatedVariantGroups(related.slice(0, 4));
      })
      .catch(() => {
        if (!cancelled) {
          setRelatedVariantGroups([]);
          void galleryApi.prewarmFingerprints({ relativePaths: [image.relative_path], limit: 1 }).catch(() => undefined);
        }
      });
    return () => {
      cancelled = true;
    };
  }, [image.relative_path, image.subfolder]);
              ...
              <>
                ...
                {relatedVariantGroups.length ? (
                  <details className="ue-detail-disclosure">
                    ...
                    <div className="ue-detail-variant-list">
                      {relatedVariantGroups.map((group) => (
                        <div key={group.id} className="ue-detail-variant-item">
                          <img src={group.cover_image.thumb_url} alt={group.cover_image.filename} loading="lazy" decoding="async" />
                          <span>
                            <strong>{t(`variantType_${group.type}`)}</strong>
                            <small>{t("variantImageCount", { count: group.count })}</small>
                          </span>
                        </div>
                      ))}
                    </div>
        ...
        <div className="ue-lightbox-toolbar">
          ...
          {metadata ? (
            <button
              ...
              aria-label={t("loraStackApply")}
              {...getTooltipProps(t("loraStackApply"))}
            >
              <Layers size={17} />
            </button>
          ) : null}

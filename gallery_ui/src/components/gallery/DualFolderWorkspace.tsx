...
const INTERNAL_IMAGE_MIME = "application/x-universal-gallery-image";
const pageSize = 80;
...
const emptySelectionState = (): PaneSelectionState => ({
  selectedPaths: [],
  focusedPath: "",
  lastSelectedPath: "",
});
...
const dragHasInternalImage = (event: React.DragEvent<HTMLElement>) =>
  Array.from(event.dataTransfer.types ?? []).includes(INTERNAL_IMAGE_MIME);

const readInternalDragPayload = (event: React.DragEvent<HTMLElement>): DragState | null => {
  try {
    const raw = event.dataTransfer.getData(INTERNAL_IMAGE_MIME);
    return raw ? JSON.parse(raw) as DragState : null;
  } catch {
    return null;
  }
};
...
export const DualFolderWorkspace = ({
  ...
  const selectImage = (pane: PaneId, image: ImageRecord, event: React.MouseEvent) => {
    ...
    setPaneSelection(pane, (current) => {
      if (event.shiftKey && current.lastSelectedPath) {
  ...
  const handlePaneDrop = async (targetPane: PaneId, event: React.DragEvent<HTMLElement>) => {
    if (!dragHasInternalImage(event)) {
      return;
    }
    event.preventDefault();
    event.stopPropagation();
    const payload = dragState ?? readInternalDragPayload(event);
    setDragState(null);
    if (!payload) {
      return;
    }
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      } else if (event.key === "Enter" && focusedImage) {
        event.preventDefault();

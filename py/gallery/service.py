...
GALLERY_INDEX_SCHEMA_VERSION = 7
...
MAX_ARTIST_SEARCH_RESULTS = 5000
...
def _read_image_dimensions(full_path: str) -> tuple[int, int]:
    if not HAS_PIL:
        return 0, 0
    try:
        with guarded_image_open(full_path) as image:
            width, height = image.size
            return int(width or 0), int(height or 0)
    except DecompressionBombError as error:
        print(f"[Universal Extractor] image dimensions skipped ({full_path}): {error}")
        return 0, 0
    except Exception:
        return 0, 0
...
def resolve_trash_storage_path(item: dict[str, Any]) -> str:
    trash_root = os.path.realpath(os.path.abspath(TRASH_DIR))
    storage_path = os.path.realpath(os.path.abspath(os.path.join(TRASH_DIR, str(item.get("storage_path", "")))))
    try:
        if os.path.commonpath([trash_root, storage_path]) != trash_root:
            raise ValueError("trash path must stay within trash directory")
    except ValueError as error:
        raise ValueError("invalid trash storage path") from error
    return storage_path
...
def restore_trash_item(item_id: str) -> dict[str, Any]:
    ...
    kind = item.get("kind")
    storage_path = resolve_trash_storage_path(item)
...
def _is_gallery_index_corruption(error: BaseException) -> bool:
    if not isinstance(error, sqlite3.DatabaseError):
        return False
    message = str(error).lower()
    return any(marker in message for marker in GALLERY_INDEX_CORRUPTION_MARKERS)


def _unique_gallery_index_quarantine_path(path: str) -> str:
    timestamp = datetime.now().strftime("%Y%m%d-%H%M%S")
    candidate = f"{path}.corrupt-{timestamp}"
    counter = 1
    while os.path.exists(candidate):
        candidate = f"{path}.corrupt-{timestamp}-{counter}"
        counter += 1
    return candidate


def _quarantine_gallery_index_db(reason: str) -> list[str]:
    quarantined: list[str] = []
    for path in (GALLERY_INDEX_DB_FILE, f"{GALLERY_INDEX_DB_FILE}-wal", f"{GALLERY_INDEX_DB_FILE}-shm"):
        if not os.path.exists(path):
            continue
        target = _unique_gallery_index_quarantine_path(path)
        os.replace(path, target)
        quarantined.append(target)
    if quarantined:
        print(
            "[Universal Extractor] Gallery SQLite index looked corrupt and was moved aside "
            f"for rebuild: {reason}"
        )
    return quarantined


def _gallery_index_file_has_invalid_header() -> bool:
    if not os.path.exists(GALLERY_INDEX_DB_FILE) or os.path.getsize(GALLERY_INDEX_DB_FILE) == 0:
        return False
    try:
        with open(GALLERY_INDEX_DB_FILE, "rb") as file:
            header = file.read(16)
    except OSError:
        return False
...
def _record_gallery_index_migration(
    ...
    connection.execute(
        """
        INSERT OR IGNORE INTO gallery_schema_migrations(id, applied_at, from_version, to_version)
        VALUES(?, ?, ?, ?)
        """,
        (migration_id, time.time(), from_version, to_version),
    )


def _apply_gallery_index_migrations(connection: sqlite3.Connection):
    connection.execute(
        """
    ...
    if current_version < GALLERY_INDEX_SCHEMA_VERSION:
        _record_gallery_index_migration(
            connection,
            f"schema-{current_version}-to-{GALLERY_INDEX_SCHEMA_VERSION}",
            current_version,
            GALLERY_INDEX_SCHEMA_VERSION,
        )
        connection.execute(f"PRAGMA user_version = {GALLERY_INDEX_SCHEMA_VERSION}")


def _initialize_gallery_index_db(connection: sqlite3.Connection):
    connection.row_factory = sqlite3.Row
    connection.execute(f"PRAGMA busy_timeout={GALLERY_INDEX_BUSY_TIMEOUT_MS}")
    ...
    connection.execute(
        """
        CREATE TABLE IF NOT EXISTS gallery_image_fingerprints (
            relative_path TEXT PRIMARY KEY,
            ...
            filename_group_key TEXT NOT NULL DEFAULT '',
    ...
    fingerprint_columns = {row["name"] for row in connection.execute("PRAGMA table_info(gallery_image_fingerprints)").fetchall()}
    if "file_hash" not in fingerprint_columns:
        connection.execute("ALTER TABLE gallery_image_fingerprints ADD COLUMN file_hash TEXT NOT NULL DEFAULT ''")
    connection.execute("CREATE INDEX IF NOT EXISTS idx_gallery_fingerprint_file ON gallery_image_fingerprints(file_hash)")
    connection.execute("CREATE INDEX IF NOT EXISTS idx_gallery_fingerprint_visual ON gallery_image_fingerprints(visual_hash)")
    connection.execute("CREATE INDEX IF NOT EXISTS idx_gallery_fingerprint_prompt ON gallery_image_fingerprints(prompt_hash)")
    connection.execute("CREATE INDEX IF NOT EXISTS idx_gallery_fingerprint_workflow ON gallery_image_fingerprints(workflow_hash)")
    connection.execute("CREATE INDEX IF NOT EXISTS idx_gallery_fingerprint_filename_group ON gallery_image_fingerprints(filename_group_key)")
    _apply_gallery_index_migrations(connection)
    connection.commit()


def _connect_gallery_index_db() -> sqlite3.Connection:
    ensure_data_dir()
    if _gallery_index_file_has_invalid_header():
        _quarantine_gallery_index_db("invalid sqlite header")
    connection = sqlite3.connect(GALLERY_INDEX_DB_FILE, timeout=GALLERY_INDEX_BUSY_TIMEOUT_MS / 1000)
    try:
        _initialize_gallery_index_db(connection)
    except sqlite3.DatabaseError as error:
        connection.close()
        if not _is_gallery_index_corruption(error):
            raise
        _quarantine_gallery_index_db(str(error))
        connection = sqlite3.connect(GALLERY_INDEX_DB_FILE, timeout=GALLERY_INDEX_BUSY_TIMEOUT_MS / 1000)
        _initialize_gallery_index_db(connection)
    return connection
...
def _sync_auxiliary_rows(connection: sqlite3.Connection, rows: list[sqlite3.Row | dict[str, Any]], remove_paths: list[str] | None = None):
    ...
    if removed_paths:
        ...
        connection.executemany("DELETE FROM gallery_image_fingerprints WHERE relative_path = ?", [(path,) for path in removed_paths])
...
def _indexed_relative_paths(connection: sqlite3.Connection) -> set[str]:
    rows = connection.execute("SELECT relative_path FROM gallery_images").fetchall()
    return {normalize_relative_path(str(row["relative_path"])) for row in rows if str(row["relative_path"]).strip()}


def _reconcile_missing_state_paths_to_index(
    connection: sqlite3.Connection,
    *,
    create_backup: bool = True,
) -> dict[str, object]:
    indexed_paths = _indexed_relative_paths(connection)
    result = migrate_missing_image_states(indexed_paths, indexed_paths, create_backup=create_backup)
    migrated = result.get("migrated", {})
    if migrated:
        _sync_image_state_to_index_db(connection, get_image_state_map())
...
def _save_image_index_to_db(index: dict[str, Any]):
    ...
    with _connect_gallery_index_db() as connection:
        ...
        connection.execute("DELETE FROM gallery_image_fingerprints")
        ...
        _index_meta_set(connection, "auxiliary_schema_version", str(GALLERY_INDEX_SCHEMA_VERSION))
        recovery = _reconcile_missing_state_paths_to_index(connection)
...
def _build_image_filter_sql(
    ...
    return " AND ".join(where_clauses), params, fts_query


def _image_row_select_sql() -> str:
    return """
        relative_path, source_id, source_relative_path, filename, relative_dir,
        subfolder, display_subfolder, size, image_width, image_height, created_at, modified_at,
        title, category, notes, pinned, boards_text,
...
def _query_filtered_image_rows(
    connection: sqlite3.Connection,
    *,
    search: str = "",
    category: str = "",
    ...
    sort_by: str = "created_at",
    sort_order: str = "desc",
    limit: int = VARIANT_GROUP_SCAN_LIMIT,
) -> list[sqlite3.Row]:
    where_sql, params, fts_query = _build_image_filter_sql(
        search=search,
        category=category,
        subfolder=subfolder,
        board_id=board_id,
    ...
    sort_direction = "ASC" if sort_order.lower() == "asc" else "DESC"
    safe_limit = max(1, min(VARIANT_GROUP_SCAN_LIMIT, int(limit or VARIANT_GROUP_SCAN_LIMIT)))
    try:
        ...
        fallback_params = [f"%{search.strip().lower()}%" if value == fts_query else value for value in params]
        return connection.execute(
            f"""
            SELECT {_image_row_select_sql()}
            FROM gallery_images
            ...
            LIMIT ?
            """,
            [*fallback_params, safe_limit],
        ).fetchall()


def _full_path_for_index_row(row: sqlite3.Row | dict[str, Any], source: dict[str, Any]) -> str:
    source_root = str(source.get("path") or "")
    source_relative_path = normalize_relative_path(str(row["source_relative_path"] or row["filename"]))
    return os.path.join(source_root, *[part for part in source_relative_path.split("/") if part])


def _fingerprint_row_current(row: sqlite3.Row, fingerprint: sqlite3.Row | None) -> bool:
    if not fingerprint:
        return False
    return (
        str(fingerprint["version"] or "") == FINGERPRINT_VERSION
        and int(fingerprint["file_size"] or 0) == int(row["size"] or 0)
        and int(fingerprint["modified_at"] or 0) == int(row["modified_at"] or 0)
    )
...
def _sync_fingerprints_for_rows(
    ...
    *,
    limit: int = FINGERPRINT_SYNC_LIMIT,
) -> dict[str, Any]:
    if not rows:
    ...
    for row in stale_rows[: max(1, int(limit or FINGERPRINT_SYNC_LIMIT))]:
        relative_path = str(row["relative_path"])
        source = source_by_id.get(str(row["source_id"]))
        if not source:
            continue
        full_path = _full_path_for_index_row(row, source)
        try:
            fingerprint = build_image_fingerprint(full_path, str(row["filename"]))
        ...
        if fingerprint.get("error"):
            failed += 1
            last_error = str(fingerprint["error"])
        else:
            completed += 1
        connection.execute(
            """
            INSERT INTO gallery_image_fingerprints(
                relative_path, source_id, file_size, modified_at, file_hash, visual_hash,
                prompt_hash, workflow_hash, filename_group_key, error, indexed_at, version
            )
            VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(relative_path) DO UPDATE SET
                source_id = excluded.source_id,
                ...
                filename_group_key = excluded.filename_group_key,
            ...
            (
                ...
                str(row["source_id"]),
                int(row["size"] or 0),
                int(row["modified_at"] or 0),
                str(fingerprint.get("file_hash") or ""),
                str(fingerprint.get("visual_hash") or ""),
                str(fingerprint.get("prompt_hash") or ""),
                str(fingerprint.get("workflow_hash") or ""),
                str(fingerprint.get("filename_group_key") or ""),
                str(fingerprint.get("error") or ""),
                time.time(),
                FINGERPRINT_VERSION,
            ),
        )

    return {"queued": queued, "completed": completed, "failed": failed, "last_error": last_error}


def _rows_with_fingerprints(connection: sqlite3.Connection, rows: list[sqlite3.Row]) -> list[dict[str, Any]]:
    if not rows:
        return []
...
def _variant_group_id(group_type: str, key: str) -> str:
    digest = hashlib.sha1(f"{group_type}:{key}".encode("utf-8", errors="ignore")).hexdigest()[:16]
    return f"{group_type}:{digest}"


def _variant_group_title(group_type: str, key: str, items: list[dict[str, Any]]) -> str:
    first = items[0]["image"]["filename"] if items else key
    if group_type == "filename_series":
        return key or first
    if group_type == "same_prompt":
        return f"Prompt: {first}"
    if group_type == "same_workflow":
        return f"Workflow: {first}"
    if group_type == "near_duplicate":
        return f"Near duplicate: {first}"
    return f"Duplicate: {first}"


def _build_variant_groups_from_items(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    groups: list[tuple[str, str, list[dict[str, Any]], float]] = []

    def add_hash_groups(group_type: str, field: str, confidence: float):
        ...
        for key, bucket in buckets.items():
            if len(bucket) > 1:
                groups.append((group_type, key, sorted(bucket, key=lambda item: item["image"]["created_at"], reverse=True), confidence))

    add_hash_groups("exact_duplicate", "exact_key", 1.0)
    add_hash_groups("same_workflow", "workflow_hash", 0.96)
    add_hash_groups("same_prompt", "prompt_hash", 0.88)
    add_hash_groups("filename_series", "filename_group_key", 0.72)

    visual_items = [item for item in items if item.get("visual_hash")]
    parent = list(range(len(visual_items)))
    ...
    for index, item in enumerate(visual_items):
        near_buckets.setdefault(find(index), []).append(item)
    for key, bucket in near_buckets.items():
        if len(bucket) > 1:
            groups.append(
                (
                    "near_duplicate",
                    "|".join(sorted(str(item["visual_hash"]) for item in bucket))[:160] or str(key),
                    sorted(bucket, key=lambda item: item["image"]["created_at"], reverse=True),
                    0.82,
                )
            )

    deduped: dict[str, dict[str, Any]] = {}
    for group_type, key, bucket, confidence in groups:
        paths = sorted(item["image"]["relative_path"] for item in bucket)
        if len(paths) < 2:
            continue
        group_id = _variant_group_id(group_type, key + "|" + "|".join(paths))
        if group_id in deduped:
            continue
        latest = max(int(item["image"]["created_at"]) for item in bucket)
        cover = bucket[0]["image"]
        deduped[group_id] = {
            "id": group_id,
            "type": group_type,
            "title": _variant_group_title(group_type, key, bucket),
            "count": len(bucket),
            "cover_image": cover,
            "latest_created_at": latest,
            "confidence": confidence,
            "images_preview": [item["image"] for item in bucket[:4]],
            "_images": [item["image"] for item in bucket],
        }

    return sorted(deduped.values(), key=lambda group: (group["latest_created_at"], group["count"]), reverse=True)


def _variant_items_for_rows(rows: list[sqlite3.Row], fingerprint_rows: list[dict[str, Any]], source_by_id: dict[str, dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for item in fingerprint_rows:
        ...
        if not source or not fingerprint:
            continue
        image = {**_image_row_to_payload(row, source), **_image_row_state(row)}
        visual_hash = str(fingerprint["visual_hash"] or "")
        file_hash = str(fingerprint["file_hash"] or "")
        exact_key = ""
        if file_hash:
            exact_key = file_hash
        items.append(
            {
                ...
                "exact_key": exact_key,
                "prompt_hash": str(fingerprint["prompt_hash"] or ""),
                "workflow_hash": str(fingerprint["workflow_hash"] or ""),
                "filename_group_key": str(fingerprint["filename_group_key"] or ""),
            }
        )
    return items
...
def list_variant_groups(
    *,
    ...
    with _connect_gallery_index_db() as connection:
        ...
        sync_status = _sync_fingerprints_for_rows(connection, rows, source_by_id)
        fingerprint_rows = _rows_with_fingerprints(connection, rows)
        connection.commit()

    items = _variant_items_for_rows(rows, fingerprint_rows, source_by_id)
    groups = _build_variant_groups_from_items(items)
    if group_type:
        groups = [group for group in groups if group["type"] == group_type]
    public_groups = [{key: value for key, value in group.items() if key != "_images"} for group in groups[: max(1, min(200, int(limit or 80)))]]
    return {
...
def get_variant_group_images(
    *,
    ...
    source_groups = _build_variant_groups_from_items([])
    # Recompute with private images only when needed for the selected group.
    sources, _ = ensure_image_index_db_current(scope=subfolder)
    source_by_id = {source["id"]: source for source in sources}
    with _connect_gallery_index_db() as connection:
        rows = _query_filtered_image_rows(
            connection,
        ...
        fingerprint_rows = _rows_with_fingerprints(connection, rows)
    source_groups = _build_variant_groups_from_items(_variant_items_for_rows(rows, fingerprint_rows, source_by_id))
    selected = next((group for group in source_groups if group["id"] == group_id), None)
    if selected is None and groups:
        selected_public = next((group for group in groups if group["id"] == group_id), None)
        if selected_public:
            selected = {**selected_public, "_images": selected_public.get("images_preview", [])}
    images = list(selected.get("_images", [])) if selected else []
    safe_page = max(1, int(page or 1))
    safe_limit = max(1, min(120, int(limit or 60)))
    offset = (safe_page - 1) * safe_limit
    public_group = {key: value for key, value in selected.items() if key != "_images"} if selected else None
    return {
        "group": public_group,
        "images": images[offset : offset + safe_limit],
        "total": len(images),
        "page": safe_page,
        "limit": safe_limit,
    }
...
def prewarm_image_fingerprints(
    *,
    ...
    with _connect_gallery_index_db() as connection:
        if relative_paths:
            ...
            rows = _query_filtered_image_rows(
                ...
                favorites_only=favorites_only,
                color_family=color_family,
                limit=max(1, min(VARIANT_GROUP_SCAN_LIMIT, int(limit or FINGERPRINT_SYNC_LIMIT))),
            )
        status = _sync_fingerprints_for_rows(connection, rows, source_by_id, limit=limit)
        connection.commit()
    return {"ok": True, **status, "status": get_fingerprint_index_status()}


def get_fingerprint_index_status() -> dict[str, Any]:
    if not os.path.exists(GALLERY_INDEX_DB_FILE):
        return {"total": 0, "indexed": 0, "pending": 0, "failed": 0, "last_error": "", "version": FINGERPRINT_VERSION}
    with _connect_gallery_index_db() as connection:
        total = int(connection.execute("SELECT COUNT(*) AS total FROM gallery_images").fetchone()["total"])
        indexed = int(
            connection.execute(
                ...
                JOIN gallery_image_fingerprints fingerprint ON fingerprint.relative_path = image.relative_path
                WHERE fingerprint.version = ?
                  AND fingerprint.file_size = image.size
                  AND fingerprint.modified_at = image.modified_at
                """,
                (FINGERPRINT_VERSION,),
            ).fetchone()["total"]
        )
        failed_row = connection.execute(
            """
            SELECT COUNT(*) AS total
            FROM gallery_image_fingerprints
            WHERE version = ? AND error <> ''
            """,
            (FINGERPRINT_VERSION,),
        ).fetchone()
    ...
    return {
        ...
        "failed": int(failed_row["total"] if failed_row else 0),
        "last_error": str(last_error_row["error"] if last_error_row else ""),
        "version": FINGERPRINT_VERSION,
    }
                ...
                with THUMB_GENERATION_SEMAPHORE:
                    if not os.path.exists(thumb_path):
                        ...
                        try:
                            ...
                            raise ValueError(f"image exceeds configured pixel limit: {relative_path}") from error
                        finally:
...
def search_library_artists(
    ...
    limit = max(1, min(int(limit), MAX_ARTIST_SEARCH_RESULTS))
